<?php if (zm_get_option('grid_cat_b')) { ?>
<?php $display_categories =  explode(',',zm_get_option('grid_cat_b_id') ); foreach ($display_categories as $category) { ?>
	<?php query_posts( array( 'showposts' => zm_get_option('grid_cat_b_n'), 'cat' => $category, 'offset' => 0, 'post__not_in' => $do_not_duplicate ) ); ?>
	<div class="grid-cat-box wow fadeInUp" data-wow-delay="0.3s">
		<h3 class="grid-cat-title">
			...
			<a href="<?php echo get_category_link($category);?>" title="<?php echo strip_tags(category_description($category)); ?>"><?php single_cat_title(); ?><span class="more-i"><span></span><span></span><span></span></span></a>
		</h3>
		<div class="clear"></div>
		<div class="grid-cat-site grid-cat-5">
