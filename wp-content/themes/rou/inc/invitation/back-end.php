...
function baweic_settings_action_links( $links, $file ) {
	array_unshift( $links, '<a href="' . admin_url( 'admin.php?page=baweic_rand_code' ) . '">' . __( '添加邀请码', 'baweic' ) . '</a>' );
	return $links;
}
...
function baweic_settings_page() {
	settings_errors();
	add_settings_section( 'baweic_settings', __( '选项', 'baweic' ), '__return_false', 'baweic_settings' );
	add_settings_field( 'baweic_field_code', __( '添加链接', 'baweic' ), 'baweic_field_link', 'baweic_settings', 'baweic_settings' );
	add_settings_field( 'baweic_field_count', __( '链接地址', 'baweic' ), 'baweic_field_text_link', 'baweic_settings', 'baweic_settings' );
?>
	<div class="wrap">
	...
	<div class="wrap">
		...
		<table id="codes_table" class="widefat plugins datatables">
			...
			if ( isset( $baweic_options['codes'] ) && count( $baweic_options['codes'] ) > 0 ) {
				...
				echo '<tr><td colspan="4">' . sprintf( __( '无邀请码, <a href="%s">添加一个</a>', 'baweic' ), admin_url( 'admin.php?page=baweic_rand_code' ) ) . '</td></tr>';
				$empty = false;
			}
			if ( $empty ) {
				echo '<tr><td colspan="4">' . sprintf( __( '无邀请码, <a href="%s">添加一个</a>', 'baweic' ), admin_url( 'admin.php?page=baweic_rand_code' ) ) . '</td></tr>';
...
function baweic_admin_notice_noone() {
	...
	if ( ! count( $codes ) ) {
		echo '<div class="error" id="message"><p>' . sprintf( __( '目前没有可以使用的邀请码 <a href="%s">现在添加</a>&nbsp;!', 'baweic' ), admin_url( 'admin.php?page=baweic_rand_code' ) ) . '</p></div>';
	}
}