...
function zm_full_thumbnail() {
	...
	if ( get_post_meta($post->ID, 'thumbnail', true) ) {
		...
		$n = count($strResult[1]);
		if($n > 0){
			echo '<header class="full-header"><figure class="full-thumbnail"><a href="'.get_permalink().'"><img src="'.get_template_directory_uri().'/timthumb.php?src='.$strResult[1][0].'&w=900&h=350&a=t&zc=1" alt="'.$post->post_title .'" /></a></figure><div class="clear"></div>';
			echo ''.the_title( sprintf( '<h2 class="entry-title-img"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' ).'</header>';
		} else {
...
function zm_full_thumbnail_h() {
	...
	if ( get_post_meta($post->ID, 'thumbnail', true) ) {
		...
		$n = count($strResult[1]);
		if($n > 0){
			echo '<header class="full-header"><figure class="full-thumbnail"><span class="load"><a href="'.get_permalink().'"><img src="' . get_template_directory_uri() . '/img/loading.png" data-original="'.get_template_directory_uri().'/timthumb.php?src='.$strResult[1][0].'&w=900&h=350&a=t&zc=1" alt="'.$post->post_title .'" /></a></span></figure><div class="clear"></div>';
			echo ''.the_title( sprintf( '<h2 class="entry-title-img"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' ).'</header>';
		} else {
			the_title( sprintf( '<h2 class="entry-title-full"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' );
			echo '<span class="title-l"></span>';
...
function zm_widge_thumbnail() {
	...
	if ( get_post_meta($post->ID, 'widge_img', true) ) {
		...
		$n = count($strResult[1]);
		if($n > 0){
			echo '<a href="'.get_permalink().'"><img src="'.get_template_directory_uri().'/timthumb.php?src='.$strResult[1][0].'&w='.zm_get_option('img_s_w').'&h='.zm_get_option('img_s_h').'&a='.zm_get_option('crop_top').'&zc=1" alt="'.$post->post_title .'" /></a>';
		}
	}
}