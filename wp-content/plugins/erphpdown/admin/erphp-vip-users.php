...
if(current_user_can('administrator')){
	...
	?>
	<div class="wrap">
		<h2>VIP用户查询<?php if($issearch) echo '（当前查询用户：'.$_POST['username'].'）';?></h2>
		<p><?php echo '共有<strong>'.$total_trade.'</strong>个VIP用户'; ?></p>
		<div>
			<h3>批量操作</h3>
			<form method="post">给所有VIP都延长（减少）<input type="text" name="adddate" placeholder="整数天数">天的VIP权限 <input type="submit" value="确定操作" class="button"><input type="hidden" name="action" value="1"> （输入负数表示减少天数）</form><br>
			<h3>单用户查询</h3>
			...
			<tbody>
				...
				if($list) {
					foreach($list as $value){
						$typeName=$value->userType==7 ?'包月' :($value->userType==8 ?'包季' : ($value->userType==10 ?'终身' : '包年'));
