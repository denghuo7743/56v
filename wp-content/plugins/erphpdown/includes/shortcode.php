...
/////////////////////////////////////////////////www.mobantu.com   82708210@qq.com
	if($_POST && $_POST['paytype'])
	{
		...
		{
			$url=constant("erphpdown")."payment/chinabank.php?ice_money=".$wpdb->escape($_POST['ice_money']);
		}
	    ...
	    {
	        $url=constant("erphpdown")."payment/xhpay.php?ice_money=".$wpdb->escape($_POST['ice_money'])."&type=1";
	    }
	    elseif(isset($_POST['paytype']) && $paytype==10)
	    {
	        $url=constant("erphpdown")."payment/xhpay.php?ice_money=".$wpdb->escape($_POST['ice_money'])."&type=2";
	    }
	    elseif(isset($_POST['paytype']) && $paytype==8)
	    {
	        $url=constant("erphpdown")."payment/alipay_jk.php?ice_money=".$wpdb->escape($_POST['ice_money']);
	    }
	...
	<form action="" method="post" onsubmit="return checkFm();">
			...
			<table class="form-table">
				...
				<tr id="cpass" style="display:none">
					...
					<input type="radio" id="paytype7" class="paytype" name="paytype" value="7"  />财付通&nbsp;    
					<?php }?> 
					<?php if(get_option('ice_china_bank_uid')){?> 
					<input type="radio" id="paytype3" class="paytype" name="paytype" value="3" />银联支付&nbsp;    
					<?php }?> 
...
if(erphpdown_lock_url(substr(plugins_url('', __FILE__),'-18','-9'),'cvuejz') != 'ZXJwaHBkb3duLS0tY3Z1ZWp6'){die();}

//站内转账
function web_zhuanzhang() {
global $wpdb;
if(!is_user_logged_in())
{
	wp_die('请登录系统');
...
<form action="" method="post" onsubmit="return checkFm();">
        ...
        <p>您可以通过本系统，将本站余额转账给其他账户！已转账金额无法退回，请慎重！</p>
        <table class="form-table">
			<tr>
