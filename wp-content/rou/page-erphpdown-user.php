...
<div class="pagewrapper clearfix" id="profile" >
		...
		<aside class="pagesidebar" style="top:44px;">
			<ul class="pagesider-menu">
				<li <?php if($_GET["pd"]=='money' || !isset($_GET["pd"])){?>class="active"<?php }?> ><a href="?pd=money"  > 资产/充值/VIP</a></li>
				...
				<li <?php if($_GET["pd"]=='info'){?>class="active"<?php }?> ><a href="?pd=info"  > 个人信息</a></li>
		...
		<div class="pagecontent profile-content">
			...
			<div id="infocenter">
				...
				<div class="profile-form">
					...
					<dl class="dl-horizontal">
						<dt>登录名</dt>
						<dd><b><?php echo esc_attr( $current_user->user_login ); ?></b></dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>注册时间</dt>
						<dd><?php echo esc_attr( $current_user->user_registered ) ?></dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>昵称</dt>
						<dd>
							<input type="text" class="profile-input" id="mm_name" name="mm_name" value="<?php echo esc_attr( $current_user->nickname ) ?>" style="background: #f1f1f1;width: 50%;">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>邮箱</dt>
						<dd>
							<input type="text" class="profile-input" id="mm_mail" name="mm_mail" value="<?php echo esc_attr( $current_user->user_email ) ?>" style="background: #f1f1f1;width: 50%;">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>网址</dt>
						<dd>
							<input type="text" class="profile-input" id="mm_url" name="mm_url" value="<?php echo esc_attr( $current_user->user_url ) ?>" style="background: #f1f1f1;width: 50%;">
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt>简介</dt>
						<dd>
							<textarea rows="4" class="profile-input" id="mm_desc" name="mm_desc" style="background: #f1f1f1;width: 50%;"><?php echo esc_html( $current_user->description ); ?></textarea>
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt></dt>
			...
			<?php }elseif($_GET["pd"]=='money' || !isset($_GET["pd"])){ ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////个人资产
					$user_Info   = wp_get_current_user();
					$userMoney=$wpdb->get_row("select * from ".$wpdb->iceinfo." where ice_user_id=".$user_Info->ID);
					if(!$userMoney)
					{
				...
				<div style="width:100%;font-size: 16px;color: #062743;line-height: 180%;">
					<h1 style="width:100%;color:#F04243;">我的资产</h1>
                ...
                <form action="" method="post" onSubmit="return checkFm();">
                        <h1 style="border-top:1px solid #999999;width:100%;color:#F04243;font-weight: 550;">在线充值</h1>
                        <table class="form-table" style="border: 1px solid #ff3b00;">
                            <tr>
                				<td>
                				<input type="text" id="ice_money" name="ice_money" required="required" style="background: #f1f1f1;width: 80%;
"><br />（请输入一个整数金额，1人民币 = <?php echo get_option('ice_proportion_alipay').' '.get_option('ice_name_alipay')?>）
                				</td>
                            </tr>
                            <tr>
                                <td>
								...
								<input type="radio" id="paytype5" class="paytype" checked name="paytype" value="5" onclick="checkCard()" style="
                                margin: 5px 5px 5px 5px; " />支付宝&nbsp;
								<?php }?>
                ...
                <form action="" method="post">
                <h1 style="border-top:1px solid #999999;width:100%;color:#F04243;font-weight: 550;">升级VIP</h1>
                	<table class="form-table" style="border: 1px solid #ff3b00;">
                        <tr>
                            <td style="border: 1px solid #ff3b00;"><?php 
                        ...
                        <tr>
                            <td>
                                <?php if($ciphp_life_price){?><input type="radio" id="userType" name="userType" value="10" checked style="
    margin: 5px 5px 5px 5px;"/>终身会员（<?php echo $ciphp_life_price.get_option('ice_name_alipay')?> ）&nbsp;<?php }?>
                                <?php if($ciphp_year_price){?><input type="radio" id="userType" name="userType" value="9" style="
    margin: 5px 5px 5px 5px;"/>包年会员（<?php echo $ciphp_year_price.get_option('ice_name_alipay')?> ）&nbsp;<?php }?>
                                <?php if($ciphp_quarter_price){?><input type="radio" id="userType" name="userType" value="8" style="
    margin: 5px 5px 5px 5px;"/>包季会员（<?php echo $ciphp_quarter_price.get_option('ice_name_alipay')?> ）&nbsp;<?php }?>
                                <?php if($ciphp_month_price){?><input type="radio" id="userType" name="userType" value="7" style="
    margin: 5px 5px 5px 5px;"/>包月会员（<?php echo $ciphp_month_price.get_option('ice_name_alipay')?> ）<?php }?>
                            </td>
                        </tr>
                        <tr>
                    ...
                    <h1 style="width:100%;color:#F04243;">充值后请点击↑升级VIP</h1>
                </form>
                </div>
			<?php } ?>
		</div>
