...
function reply_password($atts, $content=null) {
	extract(shortcode_atts(array("notice" => '<div class="reply_pass">' . sprintf(__( '<kejian>网盘密码和压缩密码：发表评论并刷新本页面可见！</kejian>', 'begin' )) . '</div>'), $atts));
	$email = null;
	$user_ID = (int) wp_get_current_user()->ID;
	if ($user_ID > 0) {
		$email = get_userdata($user_ID)->user_email;
...
function begin_register_tinymce_button( $buttons ) {
	array_push( $buttons, "", "url", "videos", "img", "reply", "login", "password", "addcode", "addfolding", "field", "ad" );
	return $buttons;
}
...
// 网址跳转
function sites_nofollow($url) {
	$url = str_replace($url, get_template_directory_uri()."/go/".base64_encode($url),$url);
	return $url;
}
	...
	if( $cat_post_query->have_posts() ) {
		...
		while( $cat_post_query->have_posts() ) {
			...
			if ( get_post_meta($post->ID, 'thumbnail', true) ) {
				...
				$thumb_img = '<img src="'.get_template_directory_uri().'/timthumb.php?src='.$strResult[1][0].'&w='.zm_get_option('img_w').'&h='.zm_get_option('img_h').'&a='.zm_get_option('crop_top').'&zc=1" alt="'.$post->post_title .'" /><div class="clear"></div>';
			}
